...
def define_optimizers(netG, netsD, BD, encoder):
    ...
    params = chain(netG.parameters(), encoder.parameters(), netsD[1].parameters(),
                   netsD[2].module.code_logits.parameters())
...
class Trainer(object):
    ...
    def train_Dnet(self, idx):
        ...
        if idx == 0:
            ...
            for i in range(self.batch_size):
                ...
                a2 = min(torch.tensor(self.n_out - 1).float().to(device),
                         torch.floor((self.n_out - 1) - ((126 - self.recp_field) - x2) / self.patch_stride)) + 1
                b1 = max(torch.tensor(0).float().to(device), torch.ceil((y1 - self.recp_field) / self.patch_stride))
                b2 = min(torch.tensor(self.n_out - 1).float().to(device),
                         torch.floor((self.n_out - 1) - ((126 - self.recp_field) - y2) / self.patch_stride)) + 1

                if (x1 != x2 and y1 != y2):
                    weights_real[i, :, a1.type(torch.int): a2.type(torch.int),
                    b1.type(torch.int): b2.type(torch.int)] = 0.0

            norm_fact_real = weights_real.sum()
            norm_fact_fake = weights_real.shape[0] * weights_real.shape[1] * weights_real.shape[2] * weights_real.shape[
                3]
    ...
    def train_BD(self):
        ...
        pred_enc_z, pred_enc_b, pred_enc_p, pred_enc_c = self.BD(self.real_img, self.fake_z.detach(),
                                                                 self.fake_b.detach(), self.fake_p.detach(),
                                                                 self.fake_c.detach())
        pred_gen_z, pred_gen_b, pred_gen_p, pred_gen_c = self.BD(self.fake_imgs[2].detach(), self.real_z, self.real_b,
                                                                 self.real_p, self.real_c)

        real_data = [self.real_img, self.fake_z.detach(), self.fake_b.detach(), self.fake_p.detach(),
                     self.fake_c.detach()]
        ...
        D_loss = -(pred_enc_z.mean() + pred_enc_b.mean() + pred_enc_p.mean() + pred_enc_c.mean()) + (
                    pred_gen_z.mean() + pred_gen_b.mean() + pred_gen_p.mean() + pred_gen_c.mean()) + penalty * 10
        D_loss.backward()
    ...
    def train_EG(self):
        ...
        # fool BD loss
        pred_enc_z, pred_enc_b, pred_enc_p, pred_enc_c = self.BD(self.real_img, self.fake_z, self.fake_b, self.fake_p,
                                                                 self.fake_c)
        pred_gen_z, pred_gen_b, pred_gen_p, pred_gen_c = self.BD(self.fake_imgs[2], self.real_z, self.real_b,
                                                                 self.real_p, self.real_c)
        fool_BD_loss = (pred_enc_z.mean() + pred_enc_b.mean() + pred_enc_p.mean() + pred_enc_c.mean()) - (
                    pred_gen_z.mean() + pred_gen_b.mean() + pred_gen_p.mean() + pred_gen_c.mean())

        EG_loss = (p_code_loss + c_code_loss) + (bg_rf_loss + bg_class_loss) + child_rf_loss + fool_BD_loss + (
                    5 * z_pred_loss + 5 * b_pred_loss + 10 * p_pred_loss + 10 * c_pred_loss)
        EG_loss.backward()
    ...
    def train(self):
        ...
        self.optimizersD, self.optimizerBD, self.optimizerGE = define_optimizers(self.netG, self.netsD, self.BD,
                                                                                 self.encoder)
        ...
        for epoch in range(cfg.TRAIN.FIRST_MAX_EPOCH):
            ...
            for data in self.dataloader:
                ...
                # prepare data
                self.real_img126, self.real_img, self.real_z, self.real_b, self.real_p, self.real_c, self.warped_bbox = self.prepare_data(
                    data)

                # forward for both E and G
                self.fake_z, self.fake_b, self.fake_p, self.fake_c = self.encoder(self.real_img, 'softmax')
                self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(self.real_z, self.real_c,
                                                                                   self.real_p, self.real_b, 'code')
            ...
            with torch.no_grad():
                ...
                self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(self.code_z, self.code_c,
                                                                                   self.code_p, self.code_b, 'code')
                save_img_results(None, (self.fake_imgs + self.fg_imgs + self.mk_imgs + self.fg_mk), epoch,
                                 self.image_dir)
            self.encoder.train()
            self.netG.train()
