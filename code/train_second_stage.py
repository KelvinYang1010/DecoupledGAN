...
class Trainer(object):
    ...
    def prepare_data(self, data):
        ...
        if random.uniform(0, 1) < 0.2:
            real_p = torch.softmax(torch.FloatTensor(self.batch_size, cfg.SUPER_CATEGORIES).normal_(0, 1), dim=1).to(
                device)
        else:
    ...
    def train(self):
        ...
        for epoch in range(cfg.TRAIN.SECOND_MAX_EPOCH):
            ...
            for data in self.dataloader:
                ...
                DD_loss = self.RF_loss(fake_pred, torch.zeros_like(fake_pred)) + self.RF_loss(real_pred,
                                                                                              torch.ones_like(
                                                                                                  real_pred))
                DD_loss.backward()
